...
def classify_document(doc: fitz.Document):
    ...
    # This check runs first and terminates the process if it's a form.
    
    # Heuristic for input fields: look for lines ending in ':' or long underscores
    has_input_fields = any(line['text'].strip().endswith(':') or '____' in line['text'] for line in all_lines)
    
    # Heuristic for declaration/signature from the V2.0 spec
    ...
    # This scoring model is only run if the document was not identified as a form.
    print("Info: Document not identified as a form. Proceeding to Flyer scoring.")
    score = 0
    
    # Characteristic: Page Count
    if doc.page_count <= 2:
        score += 1
    
    # Characteristic: Structure
    if "table of contents" not in sample_text and "appendix" not in sample_text:
        score += 1

    # Characteristic: Purpose/Content
    promo_keywords = ["rsvp", "visit website", "you're invited", "party", "conference and awards"]
    if any(keyword in sample_text for keyword in promo_keywords):
        score += 2
        
    # Characteristic: Typography
    font_sizes = [line['size'] for line in all_lines if line['size'] > 0]
    if font_sizes and (max(font_sizes) / min(font_sizes) > 3): # Large variation
        score += 1
